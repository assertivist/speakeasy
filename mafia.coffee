...
class MafiaGame
    ...
    reveal_role: (player) ->
        ALL_SOCKS.emit player.nick + ' was a ' + player.role
            ...
            if quorum.length < voting.length
                ...
                if tie
                    ...
                    for p in @players
                        if p.nick == lynched
                            p.kill
                            if p.role == 'mafia' then @civ_win(p.nick)
                            if p.role != 'villager' and @reveal_roles then @reveal_role(p)
                            if p.role == 'villager' then @reveal_role(p)
                @to_be_killed = false
                @to_be_saved = false
                @investigated = false
                for p in @players
